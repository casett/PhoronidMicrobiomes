#R functions for Microbiome/Mycobiome Analysis
#By Cassie Ettinger
...
run_dada2_16S <- function(raw_data, filt_path, maxEE_F, maxEE_R, trun_F, trun_R, Taxa_DB) {
  ...
  #not truncating reads beyond based on quality bc we want to maintain most length due 
  #to ITS length variation
  out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, maxN=0, maxEE=c(maxEE_F,maxEE_R), truncLen = c(trun_F, trun_R), truncQ=2, matchIDs =TRUE, rm.phix=TRUE, compress=TRUE, multithread=TRUE, verbose = TRUE)
  head(out)
...
run_decontam_threshold <- function(dataframe, threshold, type) {
  ...
  sample_data(dataframe)$is.neg <- sample_data(dataframe)$SampleType == type
  ...
  ps.pa.neg <- prune_samples(sample_data(ps.pa)$SampleType == type, ps.pa)
  ps.pa.pos <- prune_samples(sample_data(ps.pa)$SampleType != type, ps.pa)
...
#function calculates beta diversity, an calculates ordination
...
unifrac_avgdist <- function(physeq, rare, method, n=100) {
  
  list_of_matrixs = list()
  for (i in 1:n) {
    rarefy_physeq = rarefy_even_depth(physeq, sample.size = rare, rngseed=i, replace = FALSE)
    list_of_matrixs[[i]] =phyloseq::distance(rarefy_physeq, method) }

  dist = purrr::reduce(list_of_matrixs, `+`) / length(list_of_matrixs)
  
  #return avg dist
  return(dist)
}

calculate_beta_div_unifrac <- function(beta_div_df, beta_metric, rare, method) {
  #takes in phyloseq obj unrareified, beta diversity metric to calculate, rarefaction level, ordination method
  
  dist <- unifrac_avgdist(beta_div_df, rare, beta_metric)
